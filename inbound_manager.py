...
def run(load_sheet_data):
    ...
    # 🚀 [수정] 제조사명 중복 제거 및 구글시트 순서 그대로 유지하는 리스트 생성
    m_order = []
    if not df_m.empty:
    ...
    # 🚀 [수정] 차수 숫자로 변환 후, 구글시트 제조사 순서(Categorical)와 차수(오름차순, 낮은번호 우선)로 이중 정렬
    df_c['차수'] = pd.to_numeric(df_c['차수'], errors='coerce').fillna(0).astype(int)
    
    m_id_order = []
    for mid in df_m['제조사ID'].tolist():
        if mid not in m_id_order:
            m_id_order.append(mid)
            
    df_c['제조사ID'] = pd.Categorical(df_c['제조사ID'], categories=m_id_order, ordered=True)
    df_c = df_c.sort_values(by=['제조사ID', '차수'], ascending=[True, True]) # 차수 오름차순
    ...
    # 🚀 [수정] groupby 대신 시트 순서(m_order)를 돌면서 순서대로 출력
    for m_name in m_order:
        ...
        if not group.empty:
            ...
            for _, row in group.iterrows():
                ...
                with st.container():
                    # 🚀 [수정] 배경색 제거(transparent), 테이블 구획선(border-bottom) 추가로 깔끔한 디자인 구현
                    st.markdown(f"""
                    <div style="background-color: transparent; padding: 15px; border-left: 5px solid #2E86C1; border-radius: 6px; margin-bottom: 5px; border: 1px solid #E2E8F0;">
                        <table style="width:100%; border-collapse: collapse; font-size:1.05rem; margin-bottom: 5px;">
                            <tr style="border-bottom: 1px solid #E2E8F0;">
                                <td style="padding: 10px 5px; width:25%;"><b>차수:</b> {row['차수']}차</td>
                                <td style="padding: 10px 5px; width:25%;"><b>사이즈:</b> {row.get('피트', '40FT')}</td>
                                <td colspan="2" style="padding: 10px 5px; text-align:left;">{status_html}</td>
                            </tr>
                            <tr style="border-bottom: 1px solid #E2E8F0; color:#555555; font-size:0.95rem;">
                                <td style="padding: 10px 5px;">📦 <b>입고일:</b> {inb_dt if inb_dt else "<span style='color:#A0AEC0;'>미정</span>"}</td>
                                <td style="padding: 10px 5px;">🛬 <b>입항일:</b> {arr_dt if arr_dt else "<span style='color:#A0AEC0;'>미정</span>"}</td>
                                <td style="padding: 10px 5px;">🚢 <b>출항일:</b> {dep_dt if dep_dt else "<span style='color:#A0AEC0;'>미정</span>"}</td>
                                <td style="padding: 10px 5px;">📅 <b>발주일:</b> {ord_dt if ord_dt else "<span style='color:#A0AEC0;'>미정</span>"}</td>
                            </tr>
                            <tr>
                                <td colspan="4" style="padding: 10px 5px; color:#2C3E50;">📝 <b>적요:</b> {row.get('적요','') if str(row.get('적요','')).strip() != "" else "<span style='color:#A0AEC0;'>없음</span>"}</td>
                            </tr>
                        </table>
                    </div>
